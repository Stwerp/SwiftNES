...
# It converts the UKP assembly code `ukp.s` into `usb_hid_host_rom.mem`.
# It also generates a listing file `ukp.lst`.
#
# IMPORTANT: This script is cwd-independent. By default it reads/writes files
# in the same directory as this script, so running `python rom/asukp.py` from
# the repo root works on both Windows and Linux.
...
def main():
    parser = argparse.ArgumentParser(description="Assemble ukp.s into usb_hid_host_rom.mem")
    parser.add_argument("--in", dest="input", default=None, help="Input assembly file (default: rom/ukp.s)")
    parser.add_argument("--out", dest="output", default=None, help="Output .mem file (default: rom/usb_hid_host_rom.mem)")
    parser.add_argument("--lst", dest="listing", default=None, help="Output listing file (default: rom/ukp.lst)")
    args = parser.parse_args()

    base_dir = Path(__file__).resolve().parent
    asm_path = Path(args.input).resolve() if args.input else (base_dir / "ukp.s")
    mem_path = Path(args.output).resolve() if args.output else (base_dir / "usb_hid_host_rom.mem")
    lst_path = Path(args.listing).resolve() if args.listing else (base_dir / "ukp.lst")

    if not asm_path.exists():
        raise SystemExit(f"Input assembly not found: {asm_path}")
    ...
    with lst_path.open("w", encoding="utf-8") as lst_file:
    ...
    # ROM is implemented as 1024 x 4-bit.
    # Keep output deterministic: error if too long, pad with zeros if short.
    DEPTH = 1024
    if len(rom) > DEPTH:
        raise SystemExit(f"Assembled ROM too large: {len(rom)} nibbles (max {DEPTH})")
    if len(rom) < DEPTH:
        rom.extend([0] * (DEPTH - len(rom)))

    # Generate output file
    write_mem_file(rom, mem_path)


def write_mem_file(rom, mem_path: Path):
